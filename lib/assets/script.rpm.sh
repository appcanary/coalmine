...
yum_repo_path=/etc/yum.repos.d/appcanary_agent.repo
yum_repo_config_url="https://packagecloud.io/install/repositories/appcanary/agent/config_file.repo?os=${os}&dist=${major_version}&name=${host}&source=script"
...
yum install -y pygpgme --disablerepo='appcanary_agent'
...
if [ "$?" != "0" ]; then
  ...
  sed -i'' 's/repo_gpgcheck=1/repo_gpgcheck=0/' /etc/yum.repos.d/appcanary_agent.repo
fi

echo "Installing yum-utils..."
yum install -y yum-utils --disablerepo='appcanary_agent'
