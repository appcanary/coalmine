IntercomRails.config do |config|
  # == Intercom app_id
  #
  config.app_id = "uotxlvil"

  if Rails.env.production?
    config.app_id = ENV["INTERCOM_APP_ID"] || "m5y48sgf"
  end
  ...
  #
  config.api_secret = "4cpTQn3krCBcKd0XxbFDy8-ahk1Rup5WgZpgXRZW"
  ...
  #
  config.user.current = Proc.new { true_user }
  ...
  config.user.custom_data = {
    :servers_count => Proc.new { |current_user| current_user.servers.count },
